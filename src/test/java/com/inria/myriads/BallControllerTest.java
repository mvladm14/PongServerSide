...
public class BallControllerTest {
	...
	private BallCoordinates initialCoordinates = BallCoordinates.create().withX(250)
			.withY(250).build();
	
	private BallCoordinates otherCoordinates = BallCoordinates.create().withX(30)
			.withY(40).build();
	
	private BallSpeed ballSpeed = BallSpeed.create().withDeltaX(-1).withDeltaY(3).build();

	private PongBall pongBall = PongBall.create().withId(1).withDiameter(20).withBallSpeed(ballSpeed)
